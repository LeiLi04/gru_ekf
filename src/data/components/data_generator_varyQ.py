"""Range-only trajectory generator with time-varying Q (three segments).

This script mirrors `data_generator.py` but uses a piecewise-constant qc schedule
over the trajectory horizon:
    segment 1: qc * multipliers[0]
    segment 2: qc * multipliers[1]
    segment 3: qc * multipliers[2]

Motion regimes:
- matched: omega = 0
- mismatch: omega = pi/(2*L*dt) in turn windows (same maneuver model as data_generator.py)
...
Output is saved to data/raw/trajectory and data/raw/trajectory_wpi with filename:
    trajectory_{w}_N{num}_T{steps}_qc{qc}_sigmar{sigma}_varyQ.npz
If you change the maneuver schedule (e.g., L/K or randomization), the filename
appends a suffix like `_L25_K6_randWin_randDir` before `_varyQ`.
"""
...
class GeneratorConfig:
    ...
    qc_multipliers: Tuple[float, float, float] = (1.0, 10.0, 100.0)
...
def build_qc_schedule(cfg: GeneratorConfig) -> np.ndarray:
    steps = int(cfg.steps)
    seg = steps // 3
    qc_values = np.array(cfg.qc_multipliers, dtype=float) * float(cfg.qc)
    schedule = np.empty(steps, dtype=float)
    schedule[:seg] = qc_values[0]
    schedule[seg : 2 * seg] = qc_values[1]
    schedule[2 * seg :] = qc_values[2]
    return schedule


def build_Q_schedule(qc_schedule: np.ndarray, dt: float) -> np.ndarray:
    return np.stack([build_process_noise(float(qc), dt) for qc in qc_schedule], axis=0)
...
def simulate_trajectory(
    ...
    for k in range(cfg.steps - 1):
        ...
        if omega != 0.0:
            ...
            x[3] = s * vx + c * vy
        Q_k = Q_schedule[k]
        w_k = rng.multivariate_normal(np.zeros(4, dtype=float), Q_k)
        x = F @ x + w_k
        states[k + 1] = x
...
def generate_dataset_for_case(spec: CaseSpec, cfg: GeneratorConfig) -> Dict[str, np.ndarray]:
    ...
    Q_nom = build_process_noise(cfg.qc, cfg.dt)
    qc_schedule = build_qc_schedule(cfg)
    Q_schedule = build_Q_schedule(qc_schedule, cfg.dt)
    ...
    for n in range(cfg.trajectories):
        ...
        states, meas = simulate_trajectory(F, Q_schedule, cfg, spec.base_omega, rng, windows_n, signs_n)
        X[n] = states
        Y[n] = meas
    ...
    return {
        ...
        "qc_schedule": qc_schedule,
        "qc_multipliers": np.array(cfg.qc_multipliers, dtype=float),
...
def save_dataset(data: Dict[str, np.ndarray], root: Path, spec: CaseSpec) -> Path:
    ...
    filename = f"trajectory_{w_tag}_N{num}_T{steps}_qc{qc}_sigmar{sigma_r}{suffix}_varyQ.npz"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate range-only datasets with time-varying Q.")
    parser.add_argument(
        "--case",
    ...
    parser.add_argument(
        ...
        help="Three multipliers for qc schedule (low/mid/high), default: 1, 10, 100.",
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    cfg = GeneratorConfig(
        ...
        qc_multipliers=tuple(args.qc_multipliers),
    )
